...
def generate_and_store_embeddings(input_path, db_path, collection_name):
    ...
    print(f"Generating embeddings and adding {len(df)} documents to ChromaDB...")
    
    # ChromaDB supports batching. Let's batch to avoid memory issues and improve speed
    batch_size = 500
    for i in tqdm(range(0, len(df), batch_size)):
        ...
        metadatas = batch_df[['subject', 'type', 'priority', 'language']].fillna("unknown").to_dict('records')
