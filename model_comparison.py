...
def run_spacy(train_df, test_df):
    ...
    train_sub = train_df.sample(
        min(5000, len(train_df)), random_state=RANDOM_STATE)
    ...
    for i in range(5):  # 5 iterations
        losses = {}
        batches = minibatch(
            train_sub.index, size=compounding(
                4.0, 32.0, 1.001))
        for batch_indices in batches:
            ...
            for idx in batch_indices:
                ...
                cats = {
                    label_key: 1.0 if label_key == label else 0.0 for label_key in train_df[TARGET_COL].unique()}
                examples.append(
                    Example.from_dict(
                        nlp.make_doc(text), {
                            "cats": cats}))
...
def run_distilbert(train_df, test_df):
    ...
    train_sub = train_df.sample(
        min(5000, len(train_df)), random_state=RANDOM_STATE)
    test_sub = test_df.sample(min(1000, len(test_df)),
    ...
    # Prepare datasets
    train_ds = Dataset.from_dict({'text': train_sub[TEXT_COL].tolist(), 'label': [
                                 label2id[label] for label in train_sub[TARGET_COL]]})
    test_ds = Dataset.from_dict({'text': test_sub[TEXT_COL].tolist(), 'label': [
                                label2id[label] for label in test_sub[TARGET_COL]]})
