...
def clean_text_advanced(text):
    ...
    # SpaCy processing: tokenization, stopword removal, punctuation removal,
    # lemmatization
    doc = nlp(text)
    tokens = [
...
def preprocess_data(input_path, output_path):
    ...
    df['clean_subject'] = df['subject'].fillna(
        "").progress_apply(clean_text_advanced)
    df['clean_body'] = df['body'].fillna(
        "").progress_apply(clean_text_advanced)
